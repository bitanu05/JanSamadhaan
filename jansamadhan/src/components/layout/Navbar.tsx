...
export default function Navbar({ role, userName }: NavbarProps) {
  ...
  const ROLE_INFO: Record<string, { label: string; emoji: string; badgeClass: string }> = {
    CITIZEN: {
      label: tr("role_citizen"),
      emoji: "🧑",
      badgeClass: "bg-teal-50 text-teal-700 border-teal-200",
    },
    UNIVERSITY: {
      label: tr("role_university"),
      emoji: "🎓",
      badgeClass: "bg-indigo-50 text-indigo-700 border-indigo-200",
    },
    INDUSTRY: {
      label: tr("role_industry"),
      emoji: "🏭",
  ...
  const publicNavItems: NavItem[] = [
    { href: "/problems", label: tr("nav_problems") },
    { href: "/solutions", label: tr("nav_solutions") },
    { href: "/partnerships", label: tr("nav_partnerships") },
  ];

  const navItems: Record<string, NavItem[]> = {
    CITIZEN: [
      { href: "/citizen/dashboard", label: tr("nav_home") },
      { href: "/problems", label: tr("nav_problems") },
      { href: "/solutions", label: tr("nav_solutions") },
      { href: "/partnerships", label: tr("nav_partnerships") },
      { href: "/citizen/problems/new", label: tr("nav_report_problem") },
      { href: "/citizen/my-problems", label: tr("nav_my_reports") },
    ],
    UNIVERSITY: [
      { href: "/university/dashboard", label: tr("nav_home") },
      { href: "/problems", label: tr("nav_problems") },
      { href: "/solutions", label: tr("nav_solutions") },
      { href: "/partnerships", label: tr("nav_partnerships") },
      { href: "/university/my-solutions", label: tr("nav_my_solutions") },
    ],
    INDUSTRY: [
      { href: "/industry/dashboard", label: tr("nav_home") },
      { href: "/problems", label: tr("nav_problems") },
      { href: "/solutions", label: tr("nav_solutions") },
      { href: "/partnerships", label: tr("nav_partnerships") },
      { href: "/industry/my-partnerships", label: tr("nav_my_partnerships") },
    ],
    ADMIN: [
      { href: "/admin/dashboard", label: tr("nav_dashboard") },
      { href: "/problems", label: tr("nav_problems") },
      { href: "/solutions", label: tr("nav_solutions") },
      { href: "/partnerships", label: tr("nav_partnerships") },
    ],
            ...
            {isLoggedIn ? (
              <>
                ...
                <div className="hidden md:flex items-center gap-3 pl-2 border-l border-gray-200">
                  ...
                  >
                    <LogOut size={15} />
                    <span className="hidden lg:inline">{tr("nav_logout")}</span>
                  </button>
                </div>
              </>
            ) : (
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-1.5 shadow-xl animate-dropdown">
          {roleInfo && (
            <div className="px-3 py-2 mb-2 bg-gray-50 rounded-xl flex items-center justify-between">
              <span className="text-xs font-medium text-gray-500">{tr("nav_signed_in_as")}</span>
              <span
