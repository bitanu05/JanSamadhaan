...
// ── Types matching server-fetched data shapes ─────────────────────────────────
type ProblemItem = {
  id: string;
  title: string;
  description: string;
...
type SolutionItem = {
  ...
  problem: { id: string; title: string; category: string; location: string; status: string };
...
type PartnershipItem = {
  ...
  solution: {
    ...
    title: string;
    university: { id: string; name: string };
    problem: { id: string; title: string; category: string; location: string };
  };
};
    ...
    <div className="min-h-screen bg-[#FAFAF9] flex flex-col selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <div className="max-w-5xl mx-auto text-center space-y-6">
          ...
          {!session && (
            <div className="flex flex-wrap items-center justify-center gap-3.5 pt-4">
              <Link href="/signup" className="btn-primary text-base py-3.5 px-8 font-bold shadow-lg no-underline">
                <span>{tr("hero_cta_primary")}</span>
                <ArrowRight size={18} />
              </Link>
              <Link href="/login" className="btn-secondary text-base py-3.5 px-7 font-bold no-underline">
                {tr("hero_cta_login")}
              </Link>
            </div>
          )}
          ...
          {session && (
            <div className="pt-4">
              ...
              >
                <span>{tr("hero_cta_dashboard")}</span>
                <ArrowRight size={18} />
              </Link>
            </div>
          )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-1 space-y-8 w-full">
        ...
        <div className="flex border-b border-gray-200 bg-white rounded-t-2xl px-6 pt-3 gap-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] overflow-x-auto scrollbar-none">
          ...
          >
            <MapPin size={17} />
            <span>{tr("tab_problems")} ({problems.length})</span>
          </Link>
          ...
          >
            <Lightbulb size={17} />
            <span>{tr("tab_solutions")} ({solutions.length})</span>
          </Link>
          ...
          >
            <Handshake size={17} />
            <span>{tr("tab_partnerships")} ({partnerships.length})</span>
          </Link>
        </div>
          ...
          <div>
            {problems.length === 0 ? (
              <div className="card text-center py-16 text-gray-500 space-y-3">
                ...
                <h3 className="font-bold text-gray-800 text-lg">{tr("empty_problems_title")}</h3>
                <p className="text-sm text-gray-500 max-w-md mx-auto">{tr("empty_problems_sub")}</p>
              </div>
            ) : (
          ...
          <div>
            {solutions.length === 0 ? (
              <div className="card text-center py-16 text-gray-500 space-y-3">
                ...
                <h3 className="font-bold text-gray-800 text-lg">{tr("empty_solutions_title")}</h3>
                <p className="text-sm text-gray-500 max-w-md mx-auto">{tr("empty_solutions_sub")}</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-feed-enter">
                {solutions.map((sol) => (
                  <div key={sol.id} className="card card-hover flex flex-col justify-between space-y-4">
                    <div className="space-y-3">
                      ...
                      <div className="bg-[#FAFAF9] p-3 rounded-xl border border-gray-200/70 text-xs text-gray-600">
                        <span className="font-bold text-gray-800">{tr("card_problem_label")}</span>{" "}
                        {sol.problem.title}{" "}
                    ...
                    <div className="pt-3.5 border-t border-gray-100 flex items-center justify-between">
                      <span className="text-xs text-gray-500 flex items-center gap-1 font-medium">
                        ...
                        <span>
                          {sol._count.partnerships}{" "}
                          {sol._count.partnerships !== 1
                            ? tr("card_partnership_proposals_plural")
                            : tr("card_partnership_proposals")}
                        </span>
                      </span>
                      <Link href={`/solutions/${sol.id}`} className="btn-outline text-xs py-1.5 px-3.5 no-underline">
                        {tr("card_view_details")}
                      </Link>
                    </div>
                  </div>
                ))}
          ...
          <div>
            {partnerships.length === 0 ? (
              <div className="card text-center py-16 text-gray-500 space-y-3">
                ...
                <h3 className="font-bold text-gray-800 text-lg">{tr("empty_partnerships_title")}</h3>
                <p className="text-sm text-gray-500 max-w-md mx-auto">{tr("empty_partnerships_sub")}</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-feed-enter">
                {partnerships.map((p) => (
                  <div key={p.id} className="card card-hover space-y-3 flex flex-col justify-between">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        <span className="badge bg-amber-50 text-amber-800 text-xs font-bold px-2.5 py-1 rounded-full border border-amber-200">
                          {p.industry.name} {tr("card_industry_suffix")}
                        </span>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {tr("card_university_label")}{" "}
                          <span className="font-semibold text-gray-700">{p.solution.university.name}</span>{" "}
                          • {tr("card_problem_card_label")}{" "}
                    ...
                    <div className="pt-2 flex justify-end">
                      <Link href={`/solutions/${p.solution.id}`} className="btn-outline text-xs py-1.5 px-3.5 no-underline">
                        {tr("card_view_solution")}
                      </Link>
                    </div>
                  </div>
                ))}
      ...
      <footer className="bg-white border-t border-gray-200/80 py-8 mt-16">
        <div className="max-w-7xl mx-auto px-4 text-center text-xs text-gray-500 space-y-1">
          <p className="font-semibold text-gray-700">{tr("footer_tagline")}</p>
          <p>© {new Date().getFullYear()} JanSamadhan. {tr("footer_rights")}</p>
        </div>
      </footer>
    </div>
  );
